  ...
  function toggleAnswer(questionId, optionId, isMultiple) {
    ...
    if (isMultiple) {
      ...
      if (selectedAnswers[questionId].includes(optionId)) {
        selectedAnswers[questionId] = selectedAnswers[questionId].filter(
          (id) => id !== optionId,
        );
      } else {
    ...
    questions.forEach((question) => {
      ...
      // Multiple-choice: all correct answers must be selected
        ...
        if (
          ...
          selectedOptions.every((optId) => correctOptions.includes(optId))
        ) {
          correctCount++;
        }
      }
...
<div class="flex flex-col items-center">
  ...
  <h2 class="text-lg font-medium text-gray-400 text-center">
    {exam?.name || "Quiz"}
  </h2>
      ...
      <div>
        ...
        {#each question.options as option}
          ...
          {#await Promise.resolve(question.options.filter((opt) => opt.is_correct).length > 1) then isMultipleCorrect}
            <label
              class={`flex items-center space-x-3 cursor-pointer p-1 rounded transition
            ...
            >
              <input
                ...
                onchange={() =>
                  toggleAnswer(question.id, option.id, isMultipleCorrect)}
